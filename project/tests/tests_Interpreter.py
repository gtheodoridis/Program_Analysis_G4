...
def before_tests():
    ...
    logger.info(byte_codes.keys())

def test_noop():
    interpret = Interpreter(byte_codes['Simple_noop'], byte_codes)
    (l, s, pc) = [], [], 0
    assert None == interpret.run((l, s, pc))

def test_zero():
    interpret = Interpreter(byte_codes['Simple_zero'], byte_codes)
    (l, s, pc) = [], [], 0
    assert 0 == interpret.run((l, s, pc))

def test_hundredAndTwo():
    interpret = Interpreter(byte_codes['Simple_hundredAndTwo'], byte_codes)
    (l, s, pc) = [], [], 0
    assert 102 == interpret.run((l, s, pc))

def test_identity():
    interpret = Interpreter(byte_codes['Simple_identity'], byte_codes)
    test_int = random.randint(-sys.maxsize, sys.maxsize)
    (l, s, pc) = [test_int], [], 0
    assert test_int == interpret.run((l, s, pc))

def test_add():
    interpret = Interpreter(byte_codes['Simple_add'], byte_codes)
...
def test_min():
    interpret = Interpreter(byte_codes['Simple_min'], byte_codes)
    test_int1 = random.randint(-sys.maxsize, sys.maxsize)
    test_int2 = random.randint(-sys.maxsize, sys.maxsize)
    (l, s, pc) = [test_int1, test_int2], [], 0
...
def test_factorial():
    interpret = Interpreter(byte_codes['Simple_factorial'], byte_codes)
...
def test_helloWorld():
    interpret = Interpreter(byte_codes['Calls_helloWorld'], byte_codes)
    (l, s, pc) = [], [], 0
    assert None == interpret.run((l, s, pc))

def test_fib():
    interpret = Interpreter(byte_codes['Calls_fib'], byte_codes)
...
def test_first():
    interpret = Interpreter(byte_codes['Array_first'], byte_codes)
...
def test_access():
    interpret = Interpreter(byte_codes['Array_access'], byte_codes)
...
def test_newArray():
    interpret = Interpreter(byte_codes['Array_newArray'], byte_codes)
    (l, s, pc) = [], [], 0
    assert 1 == interpret.run((l, s, pc))

def test_bubbleSort():
    interpret = Interpreter(byte_codes['Array_bubbleSort'], byte_codes)
    test_arr = [random.randint(0, 25) for i in range(random.randint(0, 25))]
    (l, s, pc) = [0], [], 0
    interpret.memory = [test_arr]
